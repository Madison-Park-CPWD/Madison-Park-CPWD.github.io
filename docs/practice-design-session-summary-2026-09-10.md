...
## Session 5 — display layer: Looker Studio, and WeekScores widened to all three metrics

### Decisions

Storage: Sheets, already settled — no change. Display: per-student time
series (all three metrics — Grit, first-attempt success, error-reading —
plus the growth trend, not just the growth number alone) and a separate
leaderboard across all students, built in **Looker Studio** (connects
directly to the Sheet, no code for the dashboard itself — chosen over
native Sheets charts for the leaderboard/drill-down combination).

**New to-do, explicitly named, not built yet**: the leaderboard needs to
group by class, which means capturing class alongside student name —
currently only a free-text name is collected anywhere in either practice
app. Needs its own design pass (how it's captured — free text like name,
or a fixed list; where it's stored — likely threaded through the same
payload path as `student`) before the leaderboard can actually group by
class. Until then, the leaderboard is all students together.

### `WeekScores` widened from growth-trend-only to all three metrics

Previously `WeekScores` only tracked `relative_performance` per week (the
growth trend's input) — `GrowthScores`' Grit/first-attempt-success/
error-reading were single aggregate numbers per student, with no
week-by-week breakdown. Since "graphs of student performance in all
three growth numbers over time" needs exactly that, `computeExerciseStats()`
now also emits timestamped events for the other three metrics, each
anchored to when that specific signal actually happened rather than
reusing the growth trend's anchor: grit at first-solve (same anchor as
relative-performance, since both only mean something for solved
exercises), first-attempt-success at the first attempt itself (meaningful
whether or not the exercise was ever solved), error-reading at the
*second* attempt in each consecutive-failure pair (when the "repeat"
happened, not when the first failure did).

`bucketByWeek()` (renamed/generalized from the growth-trend-only
`computeWeekScores()`, same logic) now buckets all four independently,
and a new `mergeWeeklySeries()` unions the four series into one row per
week that has data from *any* metric — a student might have
relative-performance data one week but no consecutive-failure pairs that
same week, so each metric's presence/absence per week is independent, and
a missing metric in a given week is written as blank (`null`), never a
misleading `0`. `WeekScores`' header widened to `Student, Week Starting
(Mon), Relative Performance, Grit, First-Attempt Success Rate,
Error-Reading` — one row per student-week, Looker-Studio-ready as a wide
table (each metric its own chartable field). `GrowthScores` is unchanged
and already leaderboard-ready as-is (one row per student).

Verified via simulation extending the existing scenario: one solved
exercise (3 attempts, same mistake twice) and one unsolved exercise in
week 1, one first-try-solved exercise in week 2 — every value in all four
per-week series and the merged output matched hand-calculation exactly,
including the specific edge case of a metric being correctly `null` (not
`0`) in a week where that exercise's category had no qualifying data.
`GROWTH_METRICS_VERSION` bumped to `3`.

Still open: pasting this into the editor and re-running against real
data; actually connecting Looker Studio to the Sheet and building the two
views (per-student time series, leaderboard); the class-capture to-do
named above.

### Bug 5: stale header row after the WeekScores schema widened

Surfaced while connecting Looker Studio: `WeekScores`' header row was
still the old 4-column version (`Student, Week Starting (Mon), Relative
Performance (avg), Exercises This Week`) after v3's widening to 6
columns, even though the actual data rows underneath were already
correctly 6 columns wide. Root cause: `getOrCreateOutputSheet()` only
wrote the header when *creating* a brand-new sheet — since "WeekScores"
already existed from v1/v2, v3 silently kept writing new-shaped data
under the old header, so column D's label ("Exercises This Week") no
longer matched what was actually in it (Grit), and columns E/F had no
header at all. Not a Looker Studio caching issue, as first suspected —
confirmed by the user checking the actual sheet header text directly.

Fixed: `getOrCreateOutputSheet()` now (re)writes the header every call,
regardless of whether the sheet already existed — idempotent, and
prevents this exact class of bug from recurring the next time either
output sheet's schema changes. `GROWTH_METRICS_VERSION` bumped to `4`.

## Session 6 — a "Run Code" button, and why prompt text stays ungraded

Students previously had no way to see their program's actual printed
output before submitting for grading — "Run Tests" only ever showed
pass/fail plus (on failure) expected-vs-got or a traceback. Added a
second button, "Run Code," alongside a free-form stdin textarea: the
student types whatever input their program needs, one value per line,
and gets back exactly what it prints, with no judgment attached.

Available until the student's first "Run Tests" click on that exercise —
at that point both disappear for good (checked via existing per-exercise
`loadHistory`, so this holds across page reloads with no new state),
and everything works exactly as it did before this feature existed.
Never logged to attempt history, so it can't affect `attemptsTaken` or
first-attempt-success or any other growth metric — it's a pure,
read-only preview.

### Made it look like a real terminal — carefully, without touching grading

Originally, `_fake_input()` (the shim `runOneTest()` uses in place of
real `input()`) silently discarded both the prompt string and the typed
value — fine for grading, since nobody needs to see it to check
pass/fail, but it meant Run Code's output looked nothing like what
actually running the file would show. E.g. `input("What is your Name?")`
run for real prints the typed answer stuck directly onto the `?` (a
common, important lesson: the fix — a trailing space — has to go *inside*
the string, not between the parentheses) — but this was invisible in the
old fake-input output entirely.

Fixed by adding an opt-in `echoInput` flag to `runOneTest()`: when true,
`_fake_input` writes `prompt + line + "\n"` into the captured output at
the moment it's called, so Run Code's output is a faithful transcript —
prompts, typed answers, and the program's own prints, in the order they
actually happened. `runTests()`'s call site is unchanged (still omits the
flag, so it stays `false`), which means this only ever *adds* to what
`echoInput`-off runs capture — grading's `stdout`/`error` comparison
against `expected` is byte-for-byte unaffected. Checked directly whether
a bad prompt string could hide inside the old discard-everything behavior
and cause a false pass: it can't — a prompt has zero effect on what
`input()` returns, so a wrong/malformed prompt is either invisible to the
running program entirely, or a mismatched/missing quote, which is a
`SyntaxError` that already surfaces as a full traceback on every test
today (since `exec()` has to compile the whole file before any test
runs, before any prompt is even reached).

### Deliberately not grading prompt text — capitalization, punctuation, wording

Once Run Code could show a student their own prompt text, the natural
next question was whether "Run Tests" should start checking it too —
e.g. flagging a missing question mark, wrong capitalization, or a
misspelled prompt. Decided against this:

- An exact-match check against some "correct" prompt string would fail
  legitimate variations (different valid wording, punctuation, or
  capitalization) that have zero effect on program correctness — that's
  not something grading should punish.
- A fuzzier check (spellcheck/punctuation-aware grading of free-text
  prompt strings) was considered and rejected as disproportionate — not
  worth building for something purely cosmetic.
- Accepted consequence: real mistakes in a student's own prompt text
  (missing punctuation, wrong capitalization, a wrong number/date typed
  into a prompt, a missing question mark, etc.) are never caught by
  automated grading, pass or fail. The only way such issues become
  visible to the student is if they choose to run "Run Code" and read
  the echoed output themselves — there is no automated check for this
  class of mistake, by design, and none is planned.

This is also why exercise starter code should keep the existing
convention of never pre-writing a prompt string for the student (checked
`units/01-print-input-fstrings.json` — every exercise already does this,
using bare `input()`), and why a test's `stdin` should only ever contain
the values needed to exercise the code, never prompt-related content:
